...
test('optional fists preserve legacy keys, undo skin changes and survive recovery/project/GLB export',async({page},testInfo)=>{
 ...
 await expect(page.locator('#status')).toHaveText('● Salvo neste navegador',{timeout:20000});const saved=await save('hands.riglab');expect(saved.data.version).toBe(2);expect(saved.data.state.pose.length).toBe(49);expect(saved.data.state.keys.every(k=>k.pose.length===49)).toBe(true);
