...
test('optional fists preserve legacy keys, undo skin changes and survive recovery/project/GLB export',async({page},testInfo)=>{
 ...
 const legacy=await save('legacy.riglab');expect(legacy.data.version).toBe(3);
 ...
 await expect(page.locator('#status')).toHaveText('● Salvo neste navegador',{timeout:20000});const saved=await save('hands.riglab');expect(saved.data.version).toBe(3);expect(saved.data.state.pose.length).toBe(49);expect(saved.data.state.keys.every(k=>k.pose.length===49)).toBe(true);
...
test('texture/UV export and replacement persist through project, recovery and GLB',async({page},testInfo)=>{
 await page.goto('/');await page.waitForFunction(()=>window.riglabDiagnostics?.());
 await page.locator('#textures').click();await expect(page.locator('#texture-info')).toContainText('não possui texturas');await expect(page.locator('#download-texture')).toBeDisabled();await page.locator('#close-textures').click();
 const fixture=await page.evaluate(async()=>{const T=await import('/node_modules/three/build/three.module.js'),{GLTFExporter}=await import('/node_modules/three/examples/jsm/exporters/GLTFExporter.js');const c=document.createElement('canvas');c.width=32;c.height=16;const ctx=c.getContext('2d');ctx.fillStyle='red';ctx.fillRect(0,0,32,8);ctx.fillStyle='blue';ctx.fillRect(0,8,32,8);const tex=new T.CanvasTexture(c),mesh=new T.Mesh(new T.BoxGeometry(.4,2,.3),new T.MeshStandardMaterial({map:tex}));const glb=Array.from(new Uint8Array(await new GLTFExporter().parseAsync(mesh,{binary:true})));ctx.fillStyle='#00ff00';ctx.fillRect(0,0,32,8);return {glb,png:c.toDataURL('image/png').split(',')[1]};});
 await page.locator('#model-file').setInputFiles({name:'uv-test.glb',mimeType:'model/gltf-binary',buffer:Buffer.from(fixture.glb)});await expect(page.locator('#asset-name')).toHaveText('uv-test');await page.locator('#start-rig').click();await page.locator('#auto-rig').click();await expect(page.locator('#view-state')).toHaveText('EDIÇÃO DE ANIMAÇÃO');
 await page.locator('#textures').click();await expect(page.locator('#texture-info')).toContainText('32 × 16 px');
 const fitted=await page.locator('#texture-preview').boundingBox();await page.locator('#texture-zoom-in').click();expect((await page.locator('#texture-preview').boundingBox()).width).toBeGreaterThan(fitted.width);
 const viewport=await page.locator('.texture-preview').boundingBox(),beforePan=await page.locator('#texture-preview').boundingBox();await page.mouse.move(viewport.x+viewport.width/2,viewport.y+viewport.height/2);await page.mouse.down();await page.mouse.move(viewport.x+viewport.width/2+60,viewport.y+viewport.height/2+30,{steps:5});await page.mouse.up();const afterPan=await page.locator('#texture-preview').boundingBox();expect(afterPan.x-beforePan.x).toBeCloseTo(60,0);expect(afterPan.y-beforePan.y).toBeCloseTo(30,0);
 await page.mouse.wheel(0,-200);await expect.poll(async()=> (await page.locator('#texture-preview').boundingBox()).width).toBeGreaterThan(afterPan.width);
 await page.locator('#texture-actual').click();await expect(page.locator('#texture-zoom')).toHaveText('100%');expect((await page.locator('#texture-preview').boundingBox()).width).toBeCloseTo(32,0);
 await page.locator('#texture-tools').click();await expect(page.locator('.texture-controls')).toBeHidden();await page.locator('#texture-tools').click();await page.locator('#texture-fit').click();expect((await page.locator('#texture-preview').boundingBox()).width).toBeCloseTo(fitted.width,0);
 await page.locator('#texture-mode').selectOption('islands');await page.locator('#texture-island').selectOption({index:1});await expect(page.locator('#download-island')).toBeEnabled();await page.locator('#texture-mode').selectOption('atlas');
 const dl=async(selector,name)=>{const event=page.waitForEvent('download');await page.locator(selector).click();const path=testInfo.outputPath(name);await(await event).saveAs(path);return path;};
 await page.locator('#texture-mode').selectOption('islands');await expect(page.locator('#replace-texture')).toBeHidden();const island=await dl('#download-island','island.png');expect((await fs.readFile(island)).readUInt32BE(16)).toBe(32);const chooser=page.waitForEvent('filechooser');await page.locator('#replace-island').click();await(await chooser).setFiles({name:'island-edited.png',mimeType:'image/png',buffer:Buffer.from(fixture.png,'base64')});await expect(page.locator('#restore-texture')).toBeEnabled();await page.locator('#restore-texture').click();await page.locator('#texture-mode').selectOption('atlas');
 const svg=await dl('#download-uv-svg','uv.svg');expect(await fs.readFile(svg,'utf8')).toContain('fill="none"');const uv=await dl('#download-uv','uv.png');expect((await fs.readFile(uv)).readUInt32BE(16)).toBe(32);
 const original=await dl('#download-texture','original.png');const wrongSize=await page.evaluate(()=>{const c=document.createElement('canvas');c.width=c.height=8;return c.toDataURL().split(',')[1];});await page.locator('#texture-file').setInputFiles({name:'island-in-atlas.png',mimeType:'image/png',buffer:Buffer.from(wrongSize,'base64')});await expect(page.locator('#toast')).toContainText('O mapa completo deve ter 32 × 16 px');await expect(page.locator('#texture-info')).toContainText('32 × 16 px');const unchanged=await dl('#download-texture','unchanged.png');expect(await fs.readFile(unchanged)).toEqual(await fs.readFile(original));await page.locator('#texture-file').setInputFiles({name:'corrected.png',mimeType:'image/png',buffer:Buffer.from(fixture.png,'base64')});await expect(page.locator('#toast')).toContainText('Textura aplicada');const replaced=await dl('#download-texture','replaced.png');expect(await fs.readFile(replaced)).not.toEqual(await fs.readFile(original));
 await page.locator('#restore-texture').click();const restored=await dl('#download-texture','restored.png');expect(await fs.readFile(restored)).toEqual(await fs.readFile(original));await page.locator('#texture-file').setInputFiles({name:'corrected.png',mimeType:'image/png',buffer:Buffer.from(fixture.png,'base64')});await expect(page.locator('#toast')).toContainText('Textura aplicada');await page.locator('#close-textures').click();
 await expect(page.locator('#status')).toHaveText('● Salvo neste navegador',{timeout:20000});const project=await dl('#save-project','textures.riglab');await expect(page.locator('#open-project')).toBeEnabled();await page.reload();await page.locator('#restore-recovery').click();await expect(page.locator('#asset-meta')).toContainText('19 juntas');await page.locator('#textures').click();const recovered=await dl('#download-texture','recovered.png');expect(await fs.readFile(recovered)).toEqual(await fs.readFile(replaced));await page.locator('#close-textures').click();
 await page.locator('#demo').click();await page.locator('#project-file').setInputFiles(project);await expect(page.locator('#asset-meta')).toContainText('19 juntas');await page.locator('#textures').click();const reopened=await dl('#download-texture','reopened.png');expect(await fs.readFile(reopened)).toEqual(await fs.readFile(replaced));await page.locator('#close-textures').click();
 const glb=await fs.readFile(await dl('#export','corrected.glb'));const pixels=await page.evaluate(async data=>{const raw=new Uint8Array(data),n=new DataView(raw.buffer).getUint32(12,true),json=JSON.parse(new TextDecoder().decode(raw.slice(20,20+n))),bv=json.bufferViews[json.images[0].bufferView],start=28+n+(bv.byteOffset||0),bitmap=await createImageBitmap(new Blob([raw.slice(start,start+bv.byteLength)]));const c=document.createElement('canvas');c.width=bitmap.width;c.height=bitmap.height;const ctx=c.getContext('2d');ctx.drawImage(bitmap,0,0);return Array.from(ctx.getImageData(1,1,1,1).data);},Array.from(glb));expect(pixels).toEqual([0,255,0,255]);
});

test('separated island crop is transparent outside UV and editing returns to source pixels',async({page})=>{
 await page.goto('/');const result=await page.evaluate(async()=>{const {cropIsland,restoreIsland}=await import('/src/uv-islands.js');const source=document.createElement('canvas');source.width=source.height=16;const ctx=source.getContext('2d');ctx.fillStyle='red';ctx.fillRect(0,0,16,16);const piece={x:4,y:4,width:8,height:8,triangles:[[[.25,.25],[.75,.25],[.25,.75]]]};const crop=cropIsland(source,piece),edit=document.createElement('canvas');edit.width=edit.height=8;const ec=edit.getContext('2d');ec.fillStyle='blue';ec.fillRect(0,0,8,8);const restored=restoreIsland(source,piece,edit),read=(c,x,y)=>Array.from(c.getContext('2d').getImageData(x,y,1,1).data);let rejected=false;try{restoreIsland(source,piece,source);}catch{rejected=true;}return {inside:read(crop,1,1),outside:read(crop,7,7),edited:read(restored,5,5),untouched:read(restored,11,11),rejected};});expect(result).toEqual({inside:[255,0,0,255],outside:[0,0,0,0],edited:[0,0,255,255],untouched:[255,0,0,255],rejected:true});
});
